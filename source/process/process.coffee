...
fluorine.Process.o::concat = (proc) ->
  @__queue = @__queue.concat(proc.__queue)


# Prepend another process' steps.
#
# preconcat:: (Process fs, Process gs) -> Process gfs
fluorine.Process.o::preconcat = (proc) ->
  @__queue = proc.__queue.concat(@__queue)
...
fluorine.Process.o::run = (result) ->
  
  # Tuple | 0 or 1
  if 2 <= arguments.length
    @__result = []
    for i of arguments
      @__result.push arguments
  else
    @__result = result
  return  if 0 is @__queue.length
  __fn = @__queue.shift()
  @__recycle_queue.push __fn
...
#
# refresh:: Process fs
fluorine.Process.o::refresh = ->
  @__recycle_queue.push @__queue.shift()  until 0 is @__queue.length
  @__queue = @__recycle_queue
  @__recycle_queue = []
